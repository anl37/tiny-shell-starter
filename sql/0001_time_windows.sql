...
-- ============================================================
-- Part 3: Backfill existing summaries with numeric windows
-- ============================================================

UPDATE public.activity_summaries
SET 
  window_start_min = parsed.start_min,
  window_end_min = parsed.end_min
FROM public.parse_time_window(time_window) AS parsed
WHERE activity_summaries.time_window IS NOT NULL
  AND activity_summaries.window_start_min IS NULL;

-- ============================================================
-- Part 4: Add same columns to location_sessions
-- ============================================================
...
-- ============================================================
-- Part 5: Create pretty view for display
-- ============================================================
...
COMMENT ON VIEW public.vw_activity_summaries_pretty IS 'Activity summaries with human-readable time labels';

-- ============================================================
-- Part 6: Enforce canonical time_of_day values
-- ============================================================

-- Check that all time_of_day values are in canonical set
DO $$
DECLARE
...
BEGIN
  ...
  WHERE time_of_day NOT IN ('morning', 'afternoon', 'evening', 'night');
  
  IF invalid_count > 0 THEN
    RAISE WARNING 'Found % location_visits with non-canonical time_of_day', invalid_count;
  END IF;
  ...
  IF invalid_count > 0 THEN
    RAISE WARNING 'Found % location_sessions with non-canonical time_of_day', invalid_count;
  END IF;
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '✓ C2 Migration Complete: Time windows canonicalized';
  RAISE NOTICE '  - Added window_start_min, window_end_min to activity_summaries';
  RAISE NOTICE '  - Added window_start_min, window_end_min to location_sessions';
  RAISE NOTICE '  - Created vw_activity_summaries_pretty view';
  RAISE NOTICE '  - Verified canonical time_of_day values';
END $$;
